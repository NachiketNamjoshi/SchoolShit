...
{	
	...
	$link = mysqli_connect("localhost:3306", $username, $password) or die('Could not connect: ' . mysqli_error()); 		//build MySQL Link
	mysqli_select_db($link,$database) or die('Could not select database');		//select database
	$Table = "";  //initialize table variable
	
	$Table.= "<table>"; //Open HTML Table
	...
	else
	{
		...
		$Table.= "<tr><td colspan='$NumFields'>Query Returned " . mysqli_num_rows($Result) . " records</td></tr>";
	}
	$Table.= "</table>";
